...
fn = fns[0][1]
assert os.path.basename(fn) == "20160426sisetuitiran23.pdf"
...
fns = sorted([(os.stat(f).st_mtime, f) for f in fns], reverse=True)
fn = fns[0][1]
assert os.path.basename(fn) == "280401sisetuitiran1gou.pdf"
...
fns = sorted([(os.stat(f).st_mtime, f) for f in fns], reverse=True)
fn = fns[0][1]
assert os.path.basename(fn) == "20150930sisetuitirankourituyoutien.pdf"
